...
class SourceAccountOwedByLoggedUserTest {
    ...
    @InjectMocks
    private SourceAccountOwedByLoggedUser rule;

    @Test
    void shouldPass() {
        // given
        when(accountService.existsByAccountNumberAndUsername(eq(SOURCE_ACCOUNT), any())).thenReturn(true);
        // when
    ...
    void shouldFail() {
        // given
        when(accountService.existsByAccountNumberAndUsername(eq(SOURCE_ACCOUNT), any())).thenReturn(false);
        // when
        Either<TransferResult, TransferDTO> result = validate();
        // then
